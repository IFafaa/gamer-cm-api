...
mod tests {
    ...
    fn test_username_too_short_fails_validation() {
        let dto = RegisterDto { username: "ab".to_string(), ..valid_dto() };
        assert!(dto.validate().is_err());
    }

    #[test]
    fn test_username_too_long_fails_validation() {
        let dto = RegisterDto { username: "a".repeat(51), ..valid_dto() };
        assert!(dto.validate().is_err());
    }

    #[test]
    fn test_invalid_email_fails_validation() {
        let dto = RegisterDto { email: "not-an-email".to_string(), ..valid_dto() };
        assert!(dto.validate().is_err());
    }

    #[test]
    fn test_password_too_short_fails_validation() {
        let dto = RegisterDto { password: "abc".to_string(), ..valid_dto() };
        assert!(dto.validate().is_err());
    }
}
